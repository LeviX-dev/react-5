// AllPaySlips.jsx — Admin: Generate & View All Employees' Payslips for a Month
// Route: /pay-slips  (no employee_id in URL)
//
// Features:
//  - Month / Year selector (default = previous month)
//  - Employee dropdown → only shows employees who do NOT yet have a payslip
//    for the selected month/year (re-fetches on month/year change)
//  - Preview modal before generating (reuses same preview API)
//  - Bottom table shows ALL generated payslips for the selected month
//  - PDF print helper (no extra library)
...
export default function AllPaySlips() {
  ...
  // employees without a payslip for the selected month
  const [unpaidEmployees, setUnpaidEmployees] = useState([]);
  const [empLoading,      setEmpLoading]      = useState(false);
  const [selEmployee,     setSelEmployee]     = useState("");   // employee id string

  // all payslips for selected month
  ...
  // ── fetch employees who have NO payslip for this month/year ─────────────────
  // Route: GET /api/payslips/available-employees?month=X&year=Y
  const fetchUnpaidEmployees = useCallback(async () => {
    setEmpLoading(true);
    setSelEmployee("");        // reset picker on month/year change
    try {
      const res = await api.get("/api/payslips/available-employees", {
        params: { month: selMonth, year: selYear },
      });
      setUnpaidEmployees(res.data?.data || []);
    } catch {
      setUnpaidEmployees([]);
    } finally {
      setEmpLoading(false);
  ...
  // ── fetch ALL payslips for this month/year ───────────────────────────────────
  // Route: GET /api/payslips/by-month?month=X&year=Y
  // (separate from the employee-specific GET /api/payslips?employee_id=X)
  const fetchMonthPayslips = useCallback(async () => {
    setPsLoading(true);
    try {
      const res = await api.get("/api/payslips/by-month", {
        params: { month: selMonth, year: selYear },
      });
  ...
  useEffect(() => {
    ...
    fetchMonthPayslips();
  }, [fetchUnpaidEmployees, fetchMonthPayslips]);
  ...
  const openPreview = async () => {
    ...
    try {
      const res = await api.get("/api/payslips/preview", {
        params: { employee_id: selEmployee, month: selMonth, year: selYear },
      });
      setPreview(res.data);
      setShowModal(true);
    } catch (err) {
  ...
  const confirmGenerate = async () => {
    ...
    try {
      ...
      setPreview(null);
      // refresh both lists
      await fetchUnpaidEmployees();
      await fetchMonthPayslips();
    } catch (err) {
  ...
  const selectedEmpLabel = unpaidEmployees.find(
    (e) => String(e.id) === String(selEmployee)
  );
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} size="lg" centered>
        <Modal.Header closeButton style={{ background: "#663399", color: "#fff" }}>
          <Modal.Title style={{ fontSize: 16 }}>
            Payslip Preview — {selectedEmpLabel?.name || `Employee #${selEmployee}`} | {MONTH_NAME} {selYear}
          </Modal.Title>
        </Modal.Header>
        ...
        <Modal.Body style={{ padding: "20px 24px" }}>
            ...
            <tbody>
              ...
              {att.absent_days > 0 && (
                <tr style={{ color: "#dc3545" }}>
                  <td>
                    Absence Deduction ({att.absent_days} day{att.absent_days > 1 ? "s" : ""} ×{" "}
                    {fmt(sal.per_day_rate)}/day)
                  </td>
              ...
              <tr className="fw-bold table-light">
                ...
                <td className="text-end">
                  {fmt((sal.policy_deductions || 0) + (sal.absence_deduction || 0))}
                </td>
              </tr>
            </tbody>
          </table>
          ...
          {/* Net */}
          <div className="rounded p-3 text-center text-white" style={{ background: "#663399" }}>
        ...
        <Modal.Footer>
          ...
          {!preview.alreadyGenerated && (
            <Button
              ...
              style={{ background: "#663399", border: "none", minWidth: 150 }}
            >
    ...
    <section>
      ...
      <Card body className="mb-3">
        <div className="d-flex flex-wrap align-items-end gap-3">
            ...
            <label className="form-label mb-1" style={{ fontSize: 12, fontWeight: 600 }}>
              ...
              {empLoading && (
                <span
                  ...
                  style={{ color: "#663399", width: 12, height: 12, borderWidth: 2 }}
                />
              )}
            </label>
            <select
            ...
            >
              ...
              {unpaidEmployees.map((emp) => (
                <option key={emp.id} value={emp.id}>
                  {emp.name || `${emp.first_name || ""} ${emp.last_name || ""}`.trim()} ({emp.staff_id || `#${emp.id}`})
                </option>
              ))}
            </select>
          </div>
          ...
          <Button
            size="sm"
            disabled={!selEmployee || previewing || empLoading}
            onClick={openPreview}
            style={{
              background: selEmployee ? "#663399" : "#adb5bd",
              border: "none",
              height: 34,
              minWidth: 170,
            }}
        ...
        {/* Info pill about remaining unpaid count */}
        {!empLoading && (
            ...
            {unpaidEmployees.length > 0 ? (
              <span>
                <span
                  ...
                  style={{ background: "#663399", fontSize: 11 }}
                >
                  {unpaidEmployees.length}
                </span>
                employee{unpaidEmployees.length > 1 ? "s" : ""} pending payslip for{" "}
                <strong>{MONTH_NAME} {selYear}</strong>
              </span>
            ) : (
              <span>
                ✅ All employees have been paid for{" "}
                <strong>{MONTH_NAME} {selYear}</strong>
              </span>
            )}
          </div>
      ...
      {/* ── All payslips table for selected month ── */}
      <Card body>
        <div className="d-flex align-items-center justify-content-between mb-2">
          <Card.Title className="mb-0" style={{ fontSize: 15 }}>
            Pay Slips — {MONTH_NAME} {selYear}
          </Card.Title>
        ...
        {psLoading ? (
          ...
          <p className="text-muted text-center py-3" style={{ fontSize: 13 }}>
            No payslips generated for {MONTH_NAME} {selYear}. Select an employee above to get started.
          </p>
        ) : (
              ...
              <tbody>
                {payslips.map((ps, idx) => (
                  <tr key={ps.id}>
                    ...
                    <td className="fw-semibold">{ps.employee_name || `Employee #${ps.employee_id}`}</td>
                    <td style={{ color: "#663399", fontWeight: 600 }}>{ps.staff_id || "-"}</td>
                    <td>{ps.salaryMonth}</td>
                    ...
                    <td className="text-end" style={{ color: "#dc3545" }}>
                      {ps.absent_days > 0 ? `− ${fmt(ps.absence_deduction)}` : "—"}
                    </td>
              ...
              {payslips.length > 0 && (
                <tfoot className="table-light fw-bold">
                  <tr>
                    ...
                    <td className="text-end">
                      {fmt(payslips.reduce((s, p) => s + Number(p.gross_salary || 0), 0))}
                    </td>
                    <td className="text-end" style={{ color: "#dc3545" }}>
                      − {fmt(payslips.reduce((s, p) => s + Number(p.absence_deduction || 0), 0))}
                    </td>
                    <td className="text-end" style={{ color: "#198754" }}>
                      {fmt(payslips.reduce((s, p) => s + Number(p.net_salary || 0), 0))}
                    </td>
                    <td colSpan={2} />
                  </tr>
                </tfoot>
